...
class EventConfig 
{
     ...
     * @psalm-return (TValue is array ? \Symfony\Config\SyliusUi\EventConfig\BlockConfig : static)
     */
    public function block(string $block_name, string|array $value = []): \Symfony\Config\SyliusUi\EventConfig\BlockConfig|static
    {
        if (!\is_array($value)) {
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('blocks', $value)) {
            $this->_usedProperties['blocks'] = true;
            $this->blocks = array_map(fn ($v) => \is_array($v) ? new \Symfony\Config\SyliusUi\EventConfig\BlockConfig($v) : $v, $value['blocks']);
            unset($value['blocks']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['blocks'])) {
            $output['blocks'] = array_map(fn ($v) => $v instanceof \Symfony\Config\SyliusUi\EventConfig\BlockConfig ? $v->toArray() : $v, $this->blocks);
        }
